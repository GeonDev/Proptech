...
public interface AlertRepository extends JpaRepository<Alert, Long> {
    ...
    @Query(value = "SELECT * FROM alert JOIN user ON user.id = alert.receive_user_id WHERE alert.read_date IS NULL AND user.username =:userName ORDER BY alert.id DESC LIMIT 10", nativeQuery = true)
